...
public class PropertyDataCleaner implements DataCleaning {
            ...
            data.removeIf(row -> {
                ...
                for (int i = 0; i < row.size(); i++) {
                    if (row.get(i).trim().isEmpty() && i != buildingAreaIndex && i != yearBuiltIndex) {
                        return true; // Found an empty cell (except BuildingArea and YearBuilt) → Remove row
                    }
                }
  ...
  public void removeOutliers(List<List<String>> data) {
    ...
    List<Integer> excludeIndices = List.of(11, 25, 26, 33); // These correspond to ParkingSpot, numberOfSharedWalls, floorLevel and numberOfLevels.
    ...
    // Define the alternative format for French month names (e.g., 05-juin-2017)
    ...
    for (int i = 0; i < data.size(); i++) {
        ...
        if (dateString != null && !dateString.isEmpty()) {
            try {
                ...
                // If parsing fails, attempt to reformat the date using French month names
                try {
                    Date parsedDate = frenchFormat.parse(dateString); // Parse using French format

                    // Reformat to the target format (dd/MM/yyyy)
                    String formattedDate = targetFormat.format(parsedDate);
                    data.get(i).set(7, formattedDate); // Replace the value in the list
